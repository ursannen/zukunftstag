...
const uint8_t MaxRuntimePerSingleDispense_s = 10;
...
void webSocketEvent(byte num, WStype_t type, uint8_t* payload, size_t length) {
  ...
  switch (type) {
    case WStype_CONNECTED:
      Serial.println("Client " + String(currentClientId) + " connected.");
      clientIds.push_back(currentClientId);
      sendDifficultyLevel();
      sendExercise(currentClientId);
      break;
    case WStype_DISCONNECTED:
      Serial.println("Client " + String(currentClientId) + " disconnected.");
      std::remove(clientIds.begin(), clientIds.end(), currentClientId);
      break;
    case WStype_TEXT:
      Serial.println("Client " + String(currentClientId) + " sent data.");
...
void writePayload(String payloadId, String param1 = "", String param2 = "", String param3 = "") {
...
void sendResultStatus(bool isResultTrue) {
  ...
  sendPayload(currentClientId);

  status = "tooLate";
  writePayload(payloadId, status, String(solution));
  for (auto clientId : clientIds) {
    if (clientId != currentClientId) {
      sendPayload(clientId);
    }
  }
}
...
void sendNewExercise() {
  createExercise();
  for (auto clientId : clientIds) {
    sendExercise(clientId);
  }
}


void sendExercise(uint8_t clientId) {
